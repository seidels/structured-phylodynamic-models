...
#------------------------------------------------------------------------------#
	...
	#cp $code_dir/sc_forward_inference.xml $output_dir/${basename}_scInf.xml
...
#------------------------------------------------------------------------------#
	...
	if [ -z "$2" ]; then
	    ...
	    # do inference
	    sbatch --time=180:00:00 --job-name="${1}_sc_${seed}" --mem-per-cpu=4000 --wrap="java -Xmx2048m -jar ~/beast2.6.jar -seed 1 -overwrite -resume -working $output_dir/${basename}_scInf.xml > $output_dir/${basename}_scInf.out " 
