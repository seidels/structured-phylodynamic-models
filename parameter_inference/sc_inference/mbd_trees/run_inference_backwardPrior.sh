...
## data dirs
data_dir="../../../data"
output_dir="$data_dir/inference_logs/sc_inf/BD_rhoSamp/backwardMigration/$1"
...
#------------------------------------------------------------------------------#
	...
	#cp $code_dir/sc_inference_backwardExp.xml $output_dir/${basename}_scInf.xml
...
#------------------------------------------------------------------------------#
	...
	# do inference 180:
	#sbatch --time=24:00:00 --job-name="${1}_sc_${seed}" --mem-per-cpu=4000 --wrap="java -Xmx2048m -jar ~/beast2.6.jar -seed 1 -overwrite -working $output_dir/${basename}_scInf.xml > $output_dir/${basename}_scInf.out "
