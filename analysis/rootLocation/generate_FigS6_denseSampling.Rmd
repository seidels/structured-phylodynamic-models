...
p <- ggplot(data = subset(dat_epi, subset = recoveries %in% c(">50%",">90%")), aes(x = case, y = as.numeric(value), col = Model, size = `n-trees`)) + 
  ...
  scale_size(name = "# Trees", breaks = c(5, 60, 100))+
  #add desired value
...
ggsave(filename = paste0(picPath, "/rootRecovery_denseSampling.pdf"), plot = p, width = 15, height =10, units = "cm")
