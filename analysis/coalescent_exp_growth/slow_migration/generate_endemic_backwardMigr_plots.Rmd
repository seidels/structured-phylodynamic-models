...
p <- ggplot(Ndat, aes(x=orderedSeed, y=log(value), col = cols)) +
 ...
 # plot the index upon which the root state changes from all 1 to all 0
  ...
  xlab("Simulation Number")+
  theme(panel.grid = element_blank(), legend.position = "top", legend.title = element_blank(), legend.background = element_rect(fill = "transparent"), legend.text = element_text(family = "Times", size = 6))+  guides(col = guide_legend(nrow = 1, override.aes = list(size = 0.1)), fill = FALSE) +
  ylim(-7, 9)
