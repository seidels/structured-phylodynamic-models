...
picPath = "~/Projects/structured-phylodynamic-models/analysis/coalescent_exp_growth/slow_migration/plots/"
```
...
mr10_samples = rexp(100000, 1) #q01_samples * N0_samples /N1_samples # forward migration
...
infResults$true_root = true_root_states[1:100, ]
...
ggsave(filename = paste0(picPath, "case",case,"_migRates_forwardExp.pdf"), width = 6.5, height = 5, units = "cm", plot = p, dpi = 300)
p #
...
p <- ggplot(Ndat, aes(x=orderedSeed, y=log(value), col = cols)) +
 ...
 # plot the index upon which the root state changes from all 1 to all 0
  ...
  scale_color_manual(values = brewer.pal(n = 2, "Set1")[1:2]) + 
  xlab("Simulation Number")+
  theme(panel.grid = element_blank(), legend.position = c(0.5, 0.9), legend.title = element_blank(), legend.background = element_rect(fill = "transparent"), legend.text = element_text(family = "Times", size = 6))+  guides(col = guide_legend(nrow = 1, override.aes = list(size = 0.1)), fill = FALSE) +
  ylim(-7, 9)
...
ggsave(filename = paste0(picPath, "case",case,"_popSizes_forwardExp.pdf"), width = 6.5, height = 5, units = "cm", plot = p, dpi = 300)
p
```
