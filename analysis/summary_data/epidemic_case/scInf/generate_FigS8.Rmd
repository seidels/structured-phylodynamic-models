...
#rm(list = ls())
case = 47
load(paste0("/Volumes/Stadler/cEvoProjects/2018-Sophie-CompBDSC/simstudy_3/data/summary_data copy/sc_inf/BD_rhoSamp/all_params_", case, ".Rdat"))
result = get(paste0("result_", case))
simParams = result[[1]]
infResults = result[[2]][which(result[[2]][,3] != 0),]
infResults[, "seed"] = as.numeric(infResults[, "seed"])
picPath = "~/Projects/Comparison_BD_SC/Master_project/analysis/summary_data/epidemic_case/scInf/plots/"
...
```{r}
ggplot(infResults[1:54, ], aes(x = treeheight, y = log(N1))) + geom_point()
```
```{r}
cor.test(x = infResults[1:54, "treeheight"], infResults[1:54, "N0"], method = "spearman")
```
...
```{r}
corr = cor.test(x = BDpopSize$popSize_1[unique(Ndat$seed)], y = Ndat[which(Ndat$variable == "N1"), "value"], method = "kendall")
label = paste0("tau == ", round(corr$estimate,digits = 2))# ", p", format.pval(pv = corr$p.value, digits = 2, eps = 0.001))

p <- ggplot(data = data.frame(simdat=log(BDpopSize$popSize_1[unique(Ndat$orderedSeed)]), infdat1=log(Ndat[which(Ndat$variable == "N1"), "value"]), 
                              infdat2=log(Ndat[which(Ndat$variable == "N0"), "value"])), aes(x=simdat, y=infdat1)) +
