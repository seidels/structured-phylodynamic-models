...
library(reshape2)
cases = c(51:53)
picPath = "../analysis/summary_data/endemic_case/rootLocation"
```
...
```{r}
dat = data.frame(recovery_5050= rep(NA, 6), recovery_6040= rep(NA, 6),recovery_9010= rep(NA, 6),sd_5050=rep(NA,6), sd_6040 = rep(NA, 6), sd_9010=rep(NA,6),n_5050 =rep(0, 6), n_6040 = rep(0, 6), n_9010 = rep(0, 6), Model = rep(NA, 6), case=c(rep("fast",2), rep("medium",2),rep("slow",2)))
```
...
locFile = paste0("../../../../simulation/SC/rootLocation_case", case, ".csv")
true_root_states = read.csv(locFile, sep = ";", header = FALSE)
colnames(true_root_states) = c("seed", "root")

#load mbd and sc inference results
load(paste0("../data/summary_data/bdpsi/SC/NodeRetype_allRateSwappers_samplingChangeTime/all_params_", case, ".Rdat"))
...
load(paste0("../data/summary_data/sc_inf/SC/all_params_", case, ".Rdat"))
...
```{r}
p <- ggplot(data = subset(dat_melted, subset = recoveries %in% c(">50%",">90%")), aes(x = case, y = as.numeric(value), shape = Model, col= `n-trees`, fill = `n-trees`)) + 
