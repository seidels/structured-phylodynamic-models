...
get_infResultsTable_sc <- function(files, param_dir){
  ...
  for (file in files){
    ...
    if("migModel_.rateMatrix_0_1" %in% colnames(estimates)){
    ...
    }else if("migModel_.rateMatrix_backward_0_1" %in% colnames(estimates)){

      infParamsTable[rowcounter, 7] = median(estimates$migModel_.rateMatrix_backward_0_1, na.rm = TRUE)
      infParamsTable[rowcounter, 8] = hdi(as.numeric(estimates$migModel_.rateMatrix_backward_0_1),credMass = 0.95)[[1]]
      infParamsTable[rowcounter, 9] = hdi(as.numeric(estimates$migModel_.rateMatrix_backward_0_1),credMass = 0.95)[[2]]

      infParamsTable[rowcounter, 10] = median(estimates$migModel_.rateMatrix_backward_1_0, na.rm = TRUE)
      infParamsTable[rowcounter, 11] = hdi(as.numeric(estimates$migModel_.rateMatrix_backward_1_0),credMass = 0.95)[[1]]
      infParamsTable[rowcounter, 12] = hdi(as.numeric(estimates$migModel_.rateMatrix_backward_1_0),credMass = 0.95)[[2]]

    }else{
      print("Check under which parameter name migration rates are reported in the BEAST log!")
    }
    ...
    if(! "migModel_.rateMatrix_forward_0_1" %in% colnames(estimates)){
      ...
      mr01_estimates = as.numeric(estimates$migModel_.rateMatrix_forward_0_1)
      mr10_estimates = as.numeric(estimates$migModel_.rateMatrix_forward_1_0)
