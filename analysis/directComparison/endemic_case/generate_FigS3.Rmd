...
case= 51
bdInfDir="../data/summary_data/bdpsi/SC/NodeRetype_allRateSwappers_samplingChangeTime/"
scInfDir="../data/summary_data/sc_inf/SC/forwardExp/"
picPath = "../analysis/directComparison/endemic_case/plots/"
```
...
dat[nrow(dat) + 1, ] = c(115, "mr10", med_bd, "mr10_lower", lower_bd, "mr10_upper", upper_bd, "SC_Prior", 115)
dat[nrow(dat) + 1, ] = c(110, "mr01", med_bd, "mr01_lower", lower_bd, "mr01_upper", upper_bd, "BD_Prior",110)
dat[nrow(dat) + 1, ] = c(115, "mr01", med_bd, "mr01_lower", lower_bd, "mr01_upper", upper_bd, "SC_Prior", 115)
...
p <- ggplot(subset(dat, variable %in% "mr01"), aes(x=as.numeric(orderedSeed), y=log(as.numeric(value)), col = Model ))+
  ...
  ylab(TeX("Log 95% HPD interval of $m_{rb}$")) +  scale_color_manual(values = c(
    '#de77ae',
    '#8e0152',
    '#7fbc41',
    '#276419')) +
...
pdf(file = paste0(picPath, "case51_inferredmr01_forwardExp.pdf"))
print(p)
dev.off()
p
```
...
```{r}
p <- ggplot(subset(dat, variable %in% "mr10"), aes(x=as.numeric(seed), y=log(as.numeric(value)), col = Model, ))+ 
  geom_point() + geom_errorbar(aes(ymin=log(as.numeric(val_lower)), ymax=log(as.numeric(val_upper))), alpha=0.4) +
  geom_hline(yintercept = log(1.0), col="black") +
  xlab("Simulation Number") + 
  ylab("Log migration rate mr10") +   scale_color_manual(values = c(
    '#762a83',
    '#af8dc3',
    '#1b7837',
    '#7fbf7b')) +
...
#+ scale_color_manual( values = viridis(2))
pdf(file = paste0(picPath, "case51_inferredmr10_forwardExp.pdf"))
print(p)
dev.off()
p
```
